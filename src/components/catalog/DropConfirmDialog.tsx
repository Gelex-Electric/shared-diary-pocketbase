...
import type { DropPayload } from '../../lib/dnd';

export interface DropRequest {
  title: string;
  detail: string;
  /** Thao tác cần ngày hiệu lực (treo/tháo) — ngày ngoài hiện trường, không phải hôm nay. */
  needsDate: boolean;
...
/**
 * Hộp thoại xác nhận sau mỗi lần thả (plan §6.1).
 *
 * Không chỉ để chống kéo nhầm: đây là chỗ DUY NHẤT nhập được ngày hiệu lực
 * thật và số biên bản. Thiếu ngày đúng thì HSN theo thời điểm sẽ sai.
 */
      ...
      <div className="vl-card w-full max-w-lg p-5 space-y-4" onClick={e => e.stopPropagation()}>
        ...
        {request.irreversible && (
          <div className="flex items-start gap-2 text-xs text-warn bg-[var(--warning-soft)] rounded p-3">
            ...
            <span>
              Thao tác này ghi vào sổ cái vật tư. Sổ cái <strong>không sửa/xoá được</strong> —
              ghi sai thì phải ghi thêm nghiệp vụ ngược lại, không xoá được vết cũ.
            </span>
          </div>
        )}
        ...
        {request.needsDate && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <label className="block">
              ...
              <input
                type="text" value={documentNo} placeholder="VD: BB-2026-014"
        ...
        <label className="block">
          ...
          <textarea
            value={note} rows={2} onChange={e => setNote(e.target.value)}
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button
            onClick={() => { setBusy(true); onConfirm({ date, documentNo, note }); }}
