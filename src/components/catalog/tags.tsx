...
import type { Asset, CatalogData, Point } from '../../lib/catalog';
...
/** Nhãn loại vật tư — công tơ tách theo model (ME-41 / ME-42 / khác). */
export function assetTypeLabel(a: { type: string; model_desc?: string }): string {
  if (a.type !== 'CONGTO') {
    return a.type === 'GP03' ? 'GP-03' : a.type === 'KHAC' ? 'Khác' : a.type;
  }
  const m = (a.model_desc || '').toUpperCase().replace(/[\s-]/g, '');
  if (m.includes('ME41')) return 'ME-41';
  if (m.includes('ME42')) return 'ME-42';
  return a.model_desc ? `CT ${a.model_desc}` : 'Công tơ';
}

const ASSET_TYPE_CLS: Record<string, string> = {
  'ME-41': 'bg-blue-50 text-blue-700 border-blue-300 dark:bg-blue-500/10 dark:text-blue-300 dark:border-blue-500/40',
  'ME-42': 'bg-cyan-50 text-cyan-700 border-cyan-300 dark:bg-cyan-500/10 dark:text-cyan-300 dark:border-cyan-500/40',
  ...
  'GP-03': 'bg-teal-50 text-teal-700 border-teal-300 dark:bg-teal-500/10 dark:text-teal-300 dark:border-teal-500/40',
  Khác: GRAY,
};

export function AssetTypeTag({ asset }: { asset: { type: string; model_desc?: string } }) {
  const label = assetTypeLabel(asset);
  // Công tơ model lạ (VD DTS27) vẫn dùng màu của nhóm công tơ, không rơi về xám
  const cls = ASSET_TYPE_CLS[label]
    ?? (asset.type === 'CONGTO' ? 'bg-indigo-50 text-indigo-700 border-indigo-300 dark:bg-indigo-500/10 dark:text-indigo-300 dark:border-indigo-500/40' : GRAY);
...
export function OverdueTag({ asset }: { asset: Asset }) {
  if (asset.type === 'GP03' || !asset.next_calibration) return null;
