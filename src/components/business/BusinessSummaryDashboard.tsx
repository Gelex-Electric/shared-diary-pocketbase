...
// Giá trị tồn tại kể cả khi component unmount/remount (chuyển tab).
...
export default function BusinessSummaryDashboard() {
  ...
  // Khối văn phòng / kinh doanh: KHÔNG khóa theo tài khoản — được phép chọn lọc
  // toàn bộ Khu công nghiệp. Mặc định hiển thị tất cả KCN.
  const [selectedAccount, setSelectedAccount] = useState<string>('all');
  const activeAreaName = ACCOUNT_MAP[selectedAccount] || 'Tất cả khu công nghiệp';
  ...
  // Handle KCN account selection — reset month & pagination để tránh trạng thái lệch
      ...
      <div className="vl-card p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-slate-500 max-w-2xl">
            Phân tích số liệu sản lượng điện, doanh thu và đối soát thanh toán trên toàn bộ khu công nghiệp.
          </p>

          {/* Bộ lọc Khu công nghiệp (khối văn phòng được chọn tự do) */}
          <div className="inline-flex items-center gap-2 mt-3 bg-white border border-slate-200 rounded-xl px-3 py-1.5 shadow-sm min-w-[260px]">
        ...
        <div className="h-[440px] w-full font-sans">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={monthlySystemLoadOfAllYears} margin={{ top: 25, right: 10, left: 10, bottom: 10 }}>
              ...
              <Tooltip
                contentStyle={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '0.6rem', color: '#222f3e', fontSize: '11px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.08)' }}
                labelStyle={{ fontWeight: 800, color: '#222f3e', fontSize: '11px' }}
