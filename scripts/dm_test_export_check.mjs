...
console.log('FOT-giả 18/09: hồ sơ phát ngược theo nửa giờ, HSN 400');
{
  ...
  const r = detectReverse(day(i => cum[i].toFixed(3), i => (i < 12 ? i * 0.4 : 4.4).toFixed(3)), 400);
  ...
  eq('dư bù 4,4 × 400 = 1.760 kVArh (> 1.000 ⇒ báo)', r.dubu.value, 1760);
  eq('dư bù đơn vị', r.dubu.unit, 'kVArh');
  eq('dư bù khung 00:00 → 05:30 (chỉ số đạt 4,4 tại mốc 11)', [r.dubu.fromTime, r.dubu.toTime], [`${D} 00:00:00`, `${D} 05:30:00`]);
}
...
console.log('Dư bù: ≥ 10 % vô công giao VÀ > 1.000 kVArh/ngày (user chốt 25/09)');
{
  /* vô công giao +1 thô mỗi khoảng × HSN; nhận tăng `nvarStep` mỗi khoảng (47 khoảng) */
  const mk = (nvarStep, hsn = 100) => detectReverse(Array.from({ length: 48 }, (_, i) => ({
  ...
  eq('940 kVArh (20 % giao) → KHÔNG báo', mk(0.2), null);
  eq('đúng 1.000 kVArh (47 × 0,2 × 106,383) → KHÔNG báo (phải LỚN HƠN)', mk(0.2, 1000 / 9.4), null);
  eq('1.001,4 kVArh → báo', mk(0.2, 106.53)?.value > 1000, true);
  const r = mk(0.3);
  eq('1.410 kVArh → báo, kèm tỷ lệ 30 %', [r.value, r.giao, Math.round(r.ratio * 100)], [1410, 4700, 30]);
  eq('ghi chú có tỷ lệ', peakNote(r), '= 30% vô công giao (4.700 kVArh) · đỉnh 30 kVArh lúc 00:00');
  eq('lượng lớn (1.175) nhưng chỉ 5 % giao → KHÔNG báo', mk(0.05, 500), null);
  eq('đúng 10 % giao và 2.350 kVArh → báo', mk(0.1, 500)?.value, 2350);
  eq('dư bù đêm FOT-giả 88 kVArh → KHÔNG báo', detectReverse(day(() => 0, i => (i < 12 ? i * 0.02 : 0.22).toFixed(3)), 400).dubu, null);
  ...
  eq('câu mô tả nêu cả hai ngưỡng', a.message.includes('≥ 10% vô công giao và > 1.000 kVArh'), true);
}
